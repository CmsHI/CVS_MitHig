...
// exception since it maps onto SkipEvent which is what we want to do here.
  ...
  for (int theindex = 1; theindex<=wrapper.number_entries(); theindex++) {
    ...
    if (0) {
      double rpix = 1000;
      double zpix = - 3000;
      if (wrapper.z(theindex)>0)
        zpix = 3000;
      double t = (zpix-wrapper.z(theindex))/wrapper.pz(theindex);
      if (t<0) {
        throw edm::Exception(edm::errors::EventCorruption)
          << "BeamHaloProducer: got negative time " << t << endl;
      }
      double x = wrapper.x(theindex)+t*wrapper.px(theindex);
      double y = wrapper.y(theindex)+t*wrapper.py(theindex);
